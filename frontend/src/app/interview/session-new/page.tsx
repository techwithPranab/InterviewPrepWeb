...
function InterviewSessionPage() {
  ...
  const fetchSession = async () => {
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || 'Failed to load interview session');
      }
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      const response = await api.post(`/interview/${sessionId}/submit`, {
  ...
  const handleSubmitAudioAnswer = async () => {
    ...
    try {
      ...
      const uploadData = await api.uploadFormData('/interview/audio/upload', formData);
      ...
      const submitResponse = await api.post(`/interview/${sessionId}/submit`, {
        questionId: currentQ.questionId,
        answer: audioAnswer,
        timeSpent,
      ...
      if (!submitResponse.success) {
        throw new Error(submitResponse.message || 'Failed to submit audio answer');
      }

      // Show evaluation feedback
      setEvaluation(submitResponse.data?.evaluation);
      setShowEvaluation(true);
      ...
      if (session) {
        ...
        updatedQuestions[currentQuestion] = {
          ...
          answer: audioAnswer,
          timeSpent,
          evaluation: submitResponse.data?.evaluation
        };
        
        setSession({
          ...session,
          questions: updatedQuestions,
          answeredQuestions: submitResponse.data?.answeredQuestions,
          progress: submitResponse.data?.progress
        });
      }
  ...
  const handleSubmitVideoAnswer = async () => {
    ...
    try {
      ...
      const uploadData = await api.uploadFormData('/interview/video/upload', formData);
      ...
      if (!submitResponse.success) {
        throw new Error(submitResponse.message || 'Failed to submit video answer');
      }
