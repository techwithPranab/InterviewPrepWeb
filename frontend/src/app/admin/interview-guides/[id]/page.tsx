...
export default function AdminGuideEditorPage() {
  ...
  const fetchGuide = async () => {
    try {
      const response = await api.get(`/admin/interview-guides/${guideId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEditMode
        ? `/admin/interview-guides/${guideId}`
        : '/admin/interview-guides';
      ...
      if (response.success) {
        ...
        setError(response.message || 'Failed to save guide');
      }
    } catch (err) {
