...
export default function InterviewConfigurationInterface() {
  ...
  const loadTemplates = async () => {
    try {
      const response = await api.get('/interviewer/templates');
      if (response.success) {
  ...
  const loadSkills = async () => {
    try {
      ...
      if (response.success) {
        setAvailableSkills(response.data?.map((skill: any) => skill.name) || []);
      }
    } catch (error) {
  ...
  const loadConfiguration = async () => {
    try {
      const response = await api.get('/interviewer/configuration');
      if (response.success) {
  ...
  const saveTemplate = async (template: InterviewTemplate) => {
    try {
      const response = template._id 
        ? await api.put(`/interviewer/templates/${template._id}`, template)
        : await api.post('/interviewer/templates', template);
  ...
  const deleteTemplate = async (templateId: string) => {
    ...
    try {
      const response = await api.delete(`/interviewer/templates/${templateId}`);
  ...
  const saveConfiguration = async () => {
    try {
      const response = await api.post('/interviewer/configuration', configuration);
