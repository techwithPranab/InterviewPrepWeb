...
export default function InterviewGuideDetailPage() {
  ...
  const fetchGuide = async () => {
    try {
      setLoading(true);
      const response = await api.get<InterviewGuide>(`/interview-guides/${guideId}`);
      console.log('Guide fetch response:', response);
      if (!response.success) {
        throw new Error(response.message || 'Failed to fetch guide');
      }

      setGuide(response.guide || null);
      setError('');
    } catch (err) {
  ...
  const handleVote = async (voteType: 'upvote' | 'downvote') => {
    try {
      ...
      const response = await api.post(`/interview-guides/${guideId}/vote`, { voteType });
  ...
  const handleAccordionChange = (questionId: string) => (_event: React.SyntheticEvent, isExpanded: boolean) => {
